...
export class Token_Map extends Map {
	...
	//TODO - would be nice with some drysoot applied here (DRY, SSoT)
	get name_symbol_lut() {
	...
	get symbol_lut_object() {
		if (this._cached_symbol_lut_object === undefined) {
			this._cached_symbol_lut_object = {};

			for (const [symbol, pattern] of this.entries()) {
				this._cached_symbol_lut_object[symbol.description] = symbol;
			}
		}

		return this._cached_symbol_lut_object;
	}

	get pattern_lut_object() {
		if (this._cached_pattern_lut_object === undefined) {
			this._cached_pattern_lut_object = {};

			for (const [symbol, pattern] of this.entries()) {
				this._cached_pattern_lut_object[symbol.description] = pattern;
			}
		}

		return this._cached_pattern_lut_object;
	}
